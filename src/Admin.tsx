...
function percent(value: number, base: number): string { return base ? `${Math.round((value / base) * 100)}%` : '0%'; }
...
export default function Admin() {
  ...
  async function changeStatus(status: AdminLead['status']) {
    ...
    setBusy(true); setError('');
    try { const updated = await updateAdminLeadStatus(token, selected.id, status); setLeads((current) => current.map((lead) => lead.id === updated.id ? updated : lead)); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Unable to update lead.'); }
    finally { setBusy(false); }
  }

  if (!token || (!leads.length && !error)) return <main className="admin-shell"><section className="admin-login"><ShieldCheck size={30} /><h1>Aivanta Lead Console</h1><p>Enter the server-side admin token configured as <code>ADMIN_TOKEN</code>.</p><input type="password" value={token} onChange={(event) => setToken(event.target.value)} placeholder="Admin token" /><button onClick={() => void load()} type="button">Open console</button>{error ? <p className="admin-error">{error}</p> : null}</section></main>;
    ...
    <main className="admin-shell">
      ...
      <div className="admin-grid">
        ...
        <section className="admin-detail">{selected ? <>
          ...
          <div className="admin-section"><small>Message / discovery context</small><pre>{selected.message}</pre></div>
        </> : <div className="admin-empty">Select a lead to inspect it.</div>}</section>
      </div>
    </main>
  );
}
