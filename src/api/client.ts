...
export async function sendChatMessage(messages: ChatMessage[]): Promise<ChatResponse> {
  ...
  const body = (await response.json().catch(() => null)) as unknown;
  if (!response.ok) { const message = body && typeof body === 'object' && 'message' in body && typeof body.message === 'string' ? body.message : 'The assistant is unavailable right now.'; throw new Error(message); }
