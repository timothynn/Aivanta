...
export async function trackEvent(name: string, metadata: Record<string, string> = {}): Promise<void> {
  try {
    await fetch(`${apiBaseUrl}/api/events`, { method: 'POST', headers: { 'content-type': 'application/json' }, keepalive: true, body: JSON.stringify({ name, path: window.location.pathname, metadata }) });
  } catch {
...
export async function generateOpportunityBrief(messages: ChatMessage[]): Promise<OpportunityBrief> {
  ...
  if (!response.ok || !body || typeof body !== 'object' || !('brief' in body)) {
    const message = body && typeof body === 'object' && 'message' in body && typeof body.message === 'string' ? body.message : 'Unable to prepare the opportunity brief right now.';
    throw new Error(message);
  }
