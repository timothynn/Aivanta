...
type ChatContext = { conversation: string; createdAt: string };
...
export function Contact() {
  const [form, setForm] = useState<LeadPayload>(() => {
    ...
    if (chat) return { ...initialForm, goals: ['Assessment'], source: 'homepage_chat_discovery', message: `Aivanta Assistant discovery conversation:\n\n${chat.conversation}\n\n\nWhat I would like Aivanta to help with: ` };
    return initialForm;
  });
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      sessionStorage.removeItem('aivanta-assessment'); sessionStorage.removeItem('aivanta-chat-context');
    ...
    <section id="contact" className="contact-section">
      <div className="container contact-inner">
        <div className="contact-copy">
          ...
          <p>Share the workflow, application, or knowledge problem you want to improve. The first step is a focused assessment, not a platform rewrite.</p>
          {assessmentContext ? <div className="assessment-context" aria-label="Assessment summary"><div><small>Your assessment</small><strong>{assessmentContext.system}</strong></div><div><small>Goal</small><strong>{assessmentContext.goal}</strong></div><div><small>Next step</small><strong>{assessmentContext.priority}</strong></div></div> : null}
          {chatContext ? <div className="chat-context-note" aria-label="Assistant discovery summary"><Icon name="bot" size={18} /><div><small>Assistant discovery</small><strong>Your conversation is attached to this enquiry.</strong></div></div> : null}
        ...
        <form className="lead-form" onSubmit={handleSubmit}>
          <div className="form-grid">
            <label>Name<input autoComplete="name" name="name" onChange={(event) => updateField('name', event.target.value)} required value={form.name} /></label>
            <label>Work email<input autoComplete="email" name="email" onChange={(event) => updateField('email', event.target.value)} required type="email" value={form.email} /></label>
            <label>Company<input autoComplete="organization" name="company" onChange={(event) => updateField('company', event.target.value)} value={form.company} /></label>
            <label>Industry<select name="industry" onChange={(event) => updateField('industry', event.target.value)} value={form.industry}><option value="">Select one</option><option>Aviation</option><option>Professional Services</option><option>Financial Services</option><option>Logistics</option><option>Enterprise Software</option><option>Other</option></select></label>
          </div>
          <fieldset><legend>What are you exploring?</legend><div className="goal-options">{goalOptions.map((goal) => <label className="goal-option" key={goal}><input checked={form.goals.includes(goal)} onChange={() => toggleGoal(goal)} type="checkbox" /><span>{goal}</span></label>)}</div></fieldset>
          <label>What should AI improve?<textarea name="message" onChange={(event) => updateField('message', event.target.value)} required rows={5} value={form.message} /></label>
          ...
          <button className="button button--primary button--form" disabled={submissionState === 'submitting'} type="submit">{submissionState === 'submitting' ? 'Sending...' : 'Start a conversation'} <Icon name="arrow" size={18} /></button>
        </form>
      </div>
    </section>
  );
