...
export class PostgresLeadStore implements LeadStore {
  ...
  async createLead(submission: LeadSubmission): Promise<LeadRecord> {
    ...
    const result = await this.pool.query<LeadRow>(
      `insert into leads (name, email, company, industry, message, goals, source, opportunity_brief, qualification_score, qualification_label, qualification_reasons)
      ...
      [submission.name, submission.email, submission.company || null, submission.industry || null, submission.message, submission.goals, submission.source, submission.opportunityBrief ? JSON.stringify(submission.opportunityBrief) : null, qualification.score, qualification.label, qualification.reasons],
    );
